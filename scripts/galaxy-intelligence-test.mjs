...
for(let i=0;i<totals.samples;i++) {
  ...
  for(const [source,ownShips] of [['civilian',false],['fleet',true]]) {
    ...
    if(!result.correct && result.attacker) assert.equal(result.attacker,'romulan','Mistaken identity must come from the supplied candidate list');
...
console.log('PASS seeded report variation, mistakes in both sources, candidate selection, delays and repeatability');
