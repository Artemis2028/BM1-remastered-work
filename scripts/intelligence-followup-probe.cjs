...
(async () => {
  ...
  await check('ship presence improves the source but excludes transit and destroyed vessels',async()=>{
    const r=await ev(()=>{const t=testBM1,s=t.state;t.startWithFaction('terran');const i=t.getSystemIndexByName('Vulcan');s.visitedSystems.push(i);const observation={day:s.day,kind:'raid',attacker:'klingon'};
      ...
      const neighbors=t.intelIdentityCandidates(i);const uncharted=s.planets.findIndex((p,j)=>!t.isReportSystemKnown(j)&&t.isChartSystemVisible(j));const anonymous=t.makeIntelReport(uncharted,observation,'anonymous',s.day);
    ...
    assert.match(r.civilian,/Civilian|civilian/);assert.match(r.fleet,/Fleet/);assert.equal(r.transit,false);assert.equal(r.destroyed,false);assert.ok(r.neighbors.includes('borg')&&r.neighbors.includes('ferengi'),JSON.stringify(r.neighbors));assert.equal(r.anonymous.systemIndex,null);assert.ok(!r.anonymous.text.includes(r.unknownName));
  });
  ...
  await check('debug repair and hull controls cannot reactivate an old disable-band clamp',async()=>{
    const rows=await ev(()=>['repair','hull 100'].map(command=>{const t=testBM1,s=t.state;t.startWithFaction('terran');s.godMode=false;const pool=t.getNpcCombatDurability(s.playership).hull;const threshold=100*t.Fleet.disableThreshold(pool)/pool;s.hull=threshold+1;s.shields=0;t.applyPlayerDamage(100000,'#fff',{combatUnits:true});const crossed=t.fleetBook().personalCondition==='disabled';t.applyDebugCommand(command);s.shields=0;const timer=s.disableGrace.until>t.gameNow();const result=t.applyPlayerDamage(1,'#fff',{combatUnits:true});return {command,crossed,timer,hull:s.hull,expected:100-100/pool,condition:t.fleetBook().personalCondition,hit:result.hullDamage};}));
    for(const r of rows){assert.ok(r.crossed&&r.timer);assert.equal(r.condition,'operational');assert.ok(Math.abs(r.hull-r.expected)<1e-9,JSON.stringify(r));}console.log('DEBUG_REPAIR_REGRESSION',JSON.stringify(rows));
  });
  await check('mistaken identities include distant and peaceful factions before routes exist',async()=>{
    const r=await ev(()=>{const t=testBM1,s=t.state;t.startWithFaction('terran');s.travelRoutes=[];const before=JSON.stringify(t.diplomacyBook());const candidates=t.intelIdentityCandidates();const emptyRoutes=s.travelRoutes.length===0;const reports=Array.from({length:800},(_,i)=>t.makeIntelReport(s.currentPlanet,{day:s.day,kind:'quiet'},`global-pool-${i}`,s.day));const allegations=reports.map(r=>r.text.match(/Ships identified as (.*?);/)?.[1]).filter(Boolean);const missing=t.makeIntelReport(s.currentPlanet,undefined,'missing',s.day);t.rebuildTravelRoutes();const after=t.intelIdentityCandidates();return {candidates,after,emptyRoutes,allegations:[...new Set(allegations)],missing,diplomacyUnchanged:before===JSON.stringify(t.diplomacyBook())};});
    assert.ok(r.emptyRoutes);assert.deepEqual(r.after,r.candidates);for(const f of ['borg','ferengi','terran','romulan'])assert.ok(r.candidates.includes(f));assert.ok(r.allegations.length>5,JSON.stringify(r));assert.equal(r.missing,null);assert.ok(r.diplomacyUnchanged);console.log('GLOBAL_IDENTITY_POOL',JSON.stringify({candidates:r.candidates,accused:r.allegations}));
  });
  await check('local attack archive records real counts consistently across forty encounters',async()=>{
    const r=await ev(async()=>{const t=testBM1,s=t.state;t.startWithFaction('terran');const baseline=s.npcShips.slice();const results=[];for(let i=0;i<40;i++){s.npcShips=baseline.slice();s.activeFleetAttack=null;t.getSystemActivity().type='battle';const before=s.npcShips.length;const spawned=t.spawnFleetAttack(s.currentPlanet,'klingon');const report=t.galaxyNewsBook().items.find(r=>r.id===s.activeFleetAttack?.id);results.push({spawned,count:s.npcShips.length-before,kind:report?.kind,text:report?.text,confidence:report?.confidence});await new Promise(r=>setTimeout(r,2));}return {results,offscreen:t.spawnFleetAttack((s.currentPlanet+1)%s.planets.length,'klingon')};});
    assert.equal(r.offscreen,false);for(const row of r.results){assert.ok(row.spawned);assert.equal(row.kind,'Local fleet attack');assert.equal(row.confidence,'Local encounter record');assert.ok(row.text.startsWith(`${row.count} Klingon attacking vessels`),JSON.stringify(row));}console.log('LOCAL_ATTACK_RECORDS',r.results.length);
  });
  await check('fleet plans remain browsable outside range and explain the transfer restriction',async()=>{
    const r=await ev(()=>{const t=testBM1,s=t.state;t.startWithFaction('terran');s.latinum=1e7;const vendor=s.stations.find(st=>t.getShipyardStock(st).length);t.setCamera(vendor.x+60000,vendor.y);t.openRemoteStationShop(vendor.id);const stock=t.getShipyardStock();t.renderFleetManager(true);const text=document.querySelector('.fleet-manager').textContent;const statuses=stock.map(ship=>t.fleetPlanStatus(ship.id));return {count:stock.length,listed:stock.every(ship=>text.includes(ship.name)),reasons:statuses.map(x=>x.reason),latinum:s.latinum};});assert.ok(r.count>0&&r.listed,JSON.stringify(r));assert.ok(r.reasons.every(reason=>/transporter range|Already licensed/i.test(reason)),JSON.stringify(r));assert.equal(r.latinum,1e7);
  });
