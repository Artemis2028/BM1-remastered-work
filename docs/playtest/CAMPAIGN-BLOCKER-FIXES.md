...
- **[verification round] `ships-economy` was a coin toss.** Per-system stock capacity is seeded from
  ...
  is stable at 29/30. The first draft of this document reported a 27/30 run as "29/30, unchanged",
...
- **`ships-economy` remains 29/30.** Its "completed delivery earns destination trust" check assumes
  docking implies proximity to the world; the 600-unit drop-off rule does not grant that. It fails
  identically on `fa7de12` (`validation/ships-economy-fa7de12.log`).
